...
export default function SupabaseReactive(path, options) {
	...
	*/
	let reactives = {
		...
		* @property {Array<SetQueueItem>} setQueue Pending $set operations (appended to when a $set occurs while another is in progress)
		*/
		...
		/**
		* Queue of pending state writes
		* Only gets populate if a $set method is called while another is executing
		*
		* @typedef {Object} SetQueueItem
		* @property {Object} data The data payload to apply
		* @property {Promise} promise The pending promise associated with the $set operation
		* @property {Function} resolve The promise resolver
		* @property {Function} reject The promise rejecter
		*/
		...
		* @param {Number} [options.retries=3] How many times to retry setting before giving up
		* @param {Number} [options.retryDelay=100] How long to wait (at minimum) between retries
		* @param {Function} [options.retryBackoff] Async backoff function timer, defaults to a linear delay based on the try number * retryDelay
		*
		...
		async $set(data, options) {
			...
			if (options.markUpdating) {
				if (reactives.$meta.isUpdating) {
					if (options.queue) { // Allowed to queue up writes
						let queueItem = {
							data,
							options,
						...
						settings.debug('$set already in progress - queuing mutation');
			...
			if (options.removeKeys) {
				if (typeof data === 'object' && data !== null) {
					settings.debug(`Skipping removing keys, data is not an object: ${data}`)
				} else {
			...
			if (options.queueDrain && reactives.$meta.setQueue.length > 0) { // Drain the remaining queue
				let lastSet = reactives.$meta.setQueue.pop(); // Take the last set item
				let remainingQueue = reactives.$meta.setQueue;
				reactives.$meta.setQueue = []; // Reset the queue

				if (remainingQueue.length > 0) {
					settings.debug('Dispose of', remainingQueue.length, 'stale states');
				...
				await reactives.$set(lastSet.data, {...options, ...lastSet.options}); // Execute the last state change against $set
		...
		*/
		async $touchRemote(data) {
			...
			let newData = data.new[settings.dataColumn];
			...
			if (newData === undefined) return settings.debug('Reject server update - newData is undefined', {
				localTimestamp: reactives.$meta.timestamp,
				serverTimestamp: dataTimestamp,
			});
